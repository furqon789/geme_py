...
font_base = max(18, W // 50)
FONT = pygame.font.SysFont("arial", font_base)
FONT_BOLD = pygame.font.SysFont("arial", font_base, bold=True)
FONT_TITLE = pygame.font.SysFont("arial", int(font_base * 1.8), bold=True)
FONT_SMALL = pygame.font.SysFont("arial", max(12, W // 80))
...
# ==================== UI SCALING ====================
# Function untuk mendapatkan posisi UI yang diskalakan
def scale_ui(ui_width, ui_height, scale_factor=0.8):
    """Mengembalikan (width, height) yang diskalakan berdasarkan ukuran layar."""
    return int(W * ui_width * scale_factor), int(H * ui_height * scale_factor)

def scale_pos(x_pct, y_pct):
    """Mengembalikan (x, y) dalam persentase layar."""
...
def draw_buildings():
    ...
    draw_building(750 - camera['x'], 510 - camera['y'], 55, 45, (196, 162, 101), (139, 105, 20))
    draw_building(820 - camera['x'], 550 - camera['y'], 50, 40, (196, 162, 101), (139, 105, 20))

    # Mining camp
    draw_building(3030 - camera['x'], 1540 - camera['y'], 70, 50, (139, 105, 20), (85, 85, 85))
    draw_building(3110 - camera['x'], 1590 - camera['y'], 60, 45, (139, 105, 20), (85, 85, 85))
    draw_building(2990 - camera['x'], 1640 - camera['y'], 55, 40, (139, 105, 20), (85, 85, 85))

    # Cave entrance - PERBAIKAN: menggunakan koordinat world - camera
    cave_x, cave_y = 4165, 1755
    pygame.draw.rect(screen, (26, 26, 46), (cave_x - camera['x'], cave_y - camera['y'], 70, 105))
    pygame.draw.arc(screen, (44, 62, 80), (cave_x - 10 - camera['x'], cave_y - camera['y'], 90, 90), math.pi, 2*math.pi, 8)

    txt1 = FONT.render('Gua Tambang', True, C_WHITE)
    txt2 = FONT_SMALL.render('Masuk untuk menambang bijih', True, C_GOLD)
    screen.blit(txt1, (cave_x - camera['x'] - txt1.get_width()//2, cave_y - camera['y'] - 15))
    screen.blit(txt2, (cave_x - camera['x'] - txt2.get_width()//2, cave_y - camera['y'] - 35))
...
def draw_hud():
    # Perbaikan: HUD menggunakan posisi yang diskalakan
    hud_height = int(H * 0.05)
    draw_rect_alpha(screen, (0, 0, 0, 180), (0, 0, W, hud_height))

    # HP Bar
    bar_width = int(W * 0.1)
    bar_height = int(H * 0.015)
    pygame.draw.rect(screen, (50, 50, 50), (int(W*0.01), int(hud_height*0.35), bar_width, bar_height), border_radius=5)
    hp_w = int(bar_width * player['hp'] / player['max_hp'])
    pygame.draw.rect(screen, C_RED if player['hp'] < 20 else C_GREEN, (int(W*0.01), int(hud_height*0.35), hp_w, bar_height), border_radius=5)
    txt = FONT_SMALL.render(f"HP {player['hp']}/{player['max_hp']}", True, C_WHITE)
    screen.blit(txt, (int(W*0.01) + bar_width + 10, int(hud_height*0.35)))

    # Coins
    txt = FONT.render(f"Koin: {player['coins']}", True, C_GOLD)
    screen.blit(txt, (int(W*0.15), int(hud_height*0.35)))

    # Inventory count
    total = sum(player['inventory'].values())
    txt = FONT.render(f"Inventaris: {total}", True, C_WHITE)
    screen.blit(txt, (int(W*0.25), int(hud_height*0.35)))

    # Controls hint
    txt = FONT_SMALL.render("WASD:Gerak | E:Interaksi | Klik:Serang | I:Inv | Q:Quest | H:Potion", True, (150, 150, 150))
    screen.blit(txt, (W - txt.get_width() - 10, int(hud_height*0.35)))
